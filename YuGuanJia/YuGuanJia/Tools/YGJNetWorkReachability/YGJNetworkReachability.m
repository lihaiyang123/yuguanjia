//
//  YGJNetworkReachability.m
//  YuGuanJia
//
//  Created by Yang on 2021/8/10.
//

#import "YGJNetworkReachability.h"
...
/// 通过状态栏判断网络类型
typedef NS_ENUM(NSInteger, YGJNetworkType) {
    
    YGJNetworkTypeUnknown,      /// 未知
    YGJNetworkTypeOffline,      /// 飞行模式
    YGJNetworkTypeWiFi,         /// Wi-Fi
    YGJNetworkTypeCellularData, /// 蜂窝
};

static void ReachabilityCallback(SCNetworkReachabilityRef target, SCNetworkReachabilityFlags flags, void* info);

typedef void(^YGJNetworkReachabilityBlock)(YGJNetworkStatus status);

@interface YGJNetworkReachability ()

@property (nonatomic, copy) YGJNetworkReachabilityBlock reachabilityBlock;
@property (nonatomic, copy) YGJNetworkPermissionsStatusHandel permissionsHandel;
@end
@implementation YGJNetworkReachability {
...
+ (instancetype)manager {
    ...
    static YGJNetworkReachability *instance;
...
+ (instancetype)reachabilityWithHostName:(NSString *)hostName {
    ...
    YGJNetworkReachability *returnValue = NULL;
...
- (void)setReachabilityStatusChangeBlock:(nullable void (^)(YGJNetworkStatus status))block {
    self.reachabilityBlock = block;
}

- (YGJNetworkStatus)currentReachabilityStatus {
    YGJNetworkStatus returnValue = YGJNetworkStatusNotReachable;
...
- (void)callBackForCurrentStatus {
    ...
    YGJNetworkStatus status = self.currentReachabilityStatus;
...
- (YGJNetworkStatus)networkStatusForFlags:(SCNetworkReachabilityFlags)flags {
    if ((flags & kSCNetworkReachabilityFlagsReachable) == 0) {
        ...
        return YGJNetworkStatusNotReachable;
    }
    YGJNetworkStatus returnValue = YGJNetworkStatusNotReachable;
    if ((flags & kSCNetworkReachabilityFlagsConnectionRequired) == 0) {
         ...
         */
        returnValue = YGJNetworkStatusWiFi;
    }
    ...
    if ((flags & kSCNetworkReachabilityFlagsIsWWAN) == kSCNetworkReachabilityFlagsIsWWAN) {
        ...
        if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 7.0) {
            ...
            if ([typeStrings4G containsObject:accessString]) {
                return YGJNetworkStatusWWAN4G;
            } else if ([typeStrings3G containsObject:accessString]) {
                return YGJNetworkStatusWWAN3G;
            } else if ([typeStrings2G containsObject:accessString]) {
                return YGJNetworkStatusWWAN2G;
            } else {
                return YGJNetworkStatusUnknown;
            }
        } else {
            return YGJNetworkStatusUnknown;
        }
...
-(BOOL)configureProxies {
    ...
    proxies = CFBridgingRelease(CFNetworkCopyProxiesForURL((__bridge CFURLRef)url,                                          (__bridge CFDictionaryRef)proxySettings));
    if (proxies.count > 0) {
...
- (void)startCheckEvent {
    ...
    if ([UIDevice currentDevice].systemVersion.floatValue < 10.0 || [self currentReachable]) {
         ...
         **/
        [self notiWithAccessibleState:YGJNetworkAuthorizationAccessible];
        return;
    }
    ...
    switch (status) {
        case kCTCellularDataRestricted: {/// 系统 API 返回 无蜂窝数据访问权限
            ...
            [self getCurrentNetworkType:^(YGJNetworkType type) {
                
                /**  若用户是通过蜂窝数据 或 WLAN 上网，走到这里来 说明权限被关闭**/
                
                if (type == YGJNetworkTypeCellularData || type == YGJNetworkTypeWiFi) {
                    
                    [self notiWithAccessibleState:YGJNetworkAuthorizationRestricted];
                } else {  /// 可能开了飞行模式，无法判断
                    [self notiWithAccessibleState:YGJNetworkAuthorizationUnknown];
                }
            }];
            break;
        }
...
/// MARK: -得到当前的网络状态
- (void)getCurrentNetworkType:(void(^)(YGJNetworkType))block {
    
    if ([self isWiFiEnable]) return block(YGJNetworkTypeWiFi);
    
    YGJNetworkType type = [self getNetworkTypeFromStatusBar];
    if (type == YGJNetworkTypeWiFi) {
...
/// 从状态栏拿网络标示比对
- (YGJNetworkType)getNetworkTypeFromStatusBar {
    NSUInteger type = 0;
    @try {
        UIApplication *app = [UIApplication sharedApplication];
        UIView *statusBar = [app valueForKeyPath:@"statusBar"];
        
        if (statusBar == nil ) return YGJNetworkTypeUnknown;
        ...
        if (isModernStatusBar) { /// 在 iPhone X 上 statusBar 属于 UIStatusBar_Modern ，需要特殊处理
            ...
            return  wifiEnable ? YGJNetworkTypeWiFi : (cellularEnable ? YGJNetworkTypeCellularData : YGJNetworkTypeOffline);
        } else { /// 传统的 statusBar
            ...
            return type == 0 ? YGJNetworkTypeOffline : (type == 5 ? YGJNetworkTypeWiFi : YGJNetworkTypeCellularData);
        }
...
/// MARK: 查询权限回调
- (void)notiWithAccessibleState:(YGJNetworkAuthorizationStatus)status {
...
static void ReachabilityCallback(SCNetworkReachabilityRef target, SCNetworkReachabilityFlags flags, void* info)
{
    YGJNetworkReachability *networkObject = (__bridge YGJNetworkReachability *)info;
